...
# Fix Chinese display issue (Windows)
...
class NMFProcessor:
    def __init__(self, w_matrix_file, metadata_file, gnps_graphml_path, output_dir, n_components=11):
        ...
        self.color_map = {}
        self.cnmf_w_path = None  # record actual W used for UMAP
    ...
    def load_data(self):
        """
        - If input is W (contains Component_* columns), use directly and record cnmf_w_path;
        - Otherwise treat as raw matrix and run NMF to obtain W/H.
        """
    ...
    def _plot_h_matrix(self):
        ...
        plt.title("CNMF Component–m/z Relationship", fontsize=14)

        xticks_itv = max(len(self.H_df.columns) // 15, 1)
        plt.xticks(range(0, len(self.H_df.columns), xticks_itv),
                   self.H_df.columns[::xticks_itv], rotation=45, fontsize=10)
    ...
    def _plot_w_matrix(self):
        # hierarchical clustering and filtering low-contribution samples
        ...
        ax.set_xlabel("Samples (Clustered)", fontsize=12)
        ax.set_ylabel("Component Contribution Ratio", fontsize=12)
        ax.set_title("Sample–Component Relationship (Clustered)", fontsize=14)
    ...
    def process_umap(self, n_neighbors=10, min_dist=1.0, random_state=42):
        ...
        print("[DEBUG] Components seen:", sorted(
            self.W_df["Top_Components"].explode().unique(),
        ...
        self.W_df.to_csv(os.path.join(self.output_dir, "all_nodes_info.csv"))
    ...
    def plot_graph(self, highlight_samples=None, display_option="None", cosine_range=None):
        ...
        if not self.edges_df.empty:
            edges = self.edges_df[
                (self.edges_df["Cosine Score"] >= cmin) &
                (self.edges_df["Cosine Score"] <= cmax)
            ]
            ...
            for _, ed in edges.iterrows():
                ...
                if n1 in self.W_df.index and n2 in self.W_df.index:
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        opacity=score,
                        hoverinfo="text",
                        hovertext=f"Cosine Score: {score:.2f}",
                        showlegend=False
                    ))
